...
**** Org-roam
     ...
     are really able to.  Unfortunately, as of [2021-10-28 ĵaŭ],
     org-roam-ui is not yet on MELPA or any package repository, so we
     need to use a local copy instead.
     ...
     #+begin_src emacs-lisp   :tangle no
       (use-package org-roam-ui
         ...
         :init (add-to-list 'load-path
                            (directory-file-name
                             (concat pn/local-lisp-directory "org-roam-ui/")))
         :config
